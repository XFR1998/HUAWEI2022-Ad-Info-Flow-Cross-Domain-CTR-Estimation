...
all_cv_scores = []
for j, clf_name in enumerate(clfs.keys()):
    model = clfs[clf_name]
    cv_scores = []
    for i, (train_index, valid_index) in enumerate(kf.split(x_train, y_train)):
        ...
        test += test_pred / (kf.n_splits*len(clfs))
        cv_scores.append(roc_auc_score(val_y, val_pred))
        all_cv_scores.append(roc_auc_score(val_y, val_pred))
...
print("%s_score_list:" % 'all_clf', all_cv_scores)
print("%s_score_mean:" % 'all_clf', np.mean(all_cv_scores))
print("%s_score_std:" % 'all_clf', np.std(all_cv_scores))


# ----------------结果保存---------------
x_test['pctr'] = test
x_test[['log_id', 'pctr']].to_csv('submission_model_fusion_avgprob.csv', index=False)